...
class MonitoringClientTest extends TestCase
{
    public function test_capture_exception_builds_expected_payload(): void
    {
        ...
        $client = new MonitoringClient('https://monitoring.example', 'mp_test', 'php', '1.0.0', 2, $transport);
    ...
    {
        $transport = new FakeTransport();
        $client = new MonitoringClient('https://monitoring.example', 'mp_test', 'php', null, 2, $transport);
    ...
    public function test_capture_event_sends_event_payload(): void
    {
        ...
        $client->captureEvent(new Event('custom', 'warning', 'algo paso', 'android', '2.5.1'));

        $this->assertSame('custom', $transport->lastPayload['type']);
        $this->assertSame('warning', $transport->lastPayload['level']);
        $this->assertSame('algo paso', $transport->lastPayload['message']);
